...
There are **five** build systems. The four desktop ones each compile `main.cpp` + Lua + stb_vorbis; the fifth, `android/`, compiles none of it — see below. Three of them are now one-line stubs over shared fragments in `../SOOB-Core/build/` — change the shared fragment, not the stub. The Win98 `build.bat` is a deliberate full copy (COMMAND.COM can't safely `call` a shared script); its one per-game line is `set NAME=Find5`.
...
| Android | `cd android && ./gradlew :app:assembleDebug` (SDK 36 + NDK 29) | `app-debug.apk` |
...
`android/` is the odd one out: it builds no C of this repo at all. It is the
APK's identity only — `def appId = 'net.dynart.find5'`, a versionCode, a
launcher icon, a manifest — and pulls the entire player (Kotlin host, JNI
bridge, NDK-built Lua) from `../SOOB-Core-Android` via the `includeBuild` in
`android/settings.gradle`. Same rule as the desktop stubs: **change the shared
fragment, not the stub**. The build logic lives in that repo's
`gradle/soobApp.gradle` and `gradle/syncGame.gradle`, and
`android/app/src/main/assets/` is generated by `syncGame` (the bundle plus
SOOB-Core's `scripts/engine`) and gitignored — never edit it. On Android the
save file is `<filesDir>/find5.dat` in the desktop format, and `config.lua` is
not copied because every field in it is desktop-only.

The Kotlin half needs a device. The C half can be booted on the desktop against
this bundle from `../SOOB-Core-Android/tools/hosttest`:
`./build.sh && ./host_test.exe ../../../Find5`.

There are no tests and no lint step for the desktop builds.
